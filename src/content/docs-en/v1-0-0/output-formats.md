...
When using `--output` with the human format, the report is written to both the file and the terminal. This allows you to save the report without losing immediate feedback.
...
- **`defaultConfiguration`**: Default level for the rule (`error`, `warning`, `note`).
...
It's possible to generate multiple reports in a single run using scripts:
