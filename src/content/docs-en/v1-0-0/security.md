...
```
1. Developer asks AI agent: "Create a REST API with JWT authentication"
...
vigil detects all 4 problems from step 2. All four analyzers are active: Dependency (DEP), Auth (AUTH), Secrets (SEC), and Test Quality (TEST).
...
- **Leaked real secrets**: Use Gitleaks or TruffleHog for that.
...
**Disadvantages:**
- Cannot detect novel problems that weren't anticipated in the rules.
...
1. **Run vigil in CI**: Automate the scan on every PR so that no AI-generated code reaches production without verification.
