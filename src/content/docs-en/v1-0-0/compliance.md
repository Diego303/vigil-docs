...
Organizations adopting AI agents (Copilot, Cursor, Claude Code, ChatGPT) to generate code face new risks that traditional SAST tools don't cover:
...
4. **Cosmetic tests**: Agents generate tests that pass but don't verify anything real.

vigil detects these 4 patterns in a **deterministic, auditable way without dependency on external AI APIs**.
...
| Adequate testing | TEST-001 to TEST-006 detect tests that don't verify anything |
...
[3. Snyk/Dependabot]     <- CVEs in dependencies
    |
    v
[4. Gitleaks]            <- Leaked real secrets
    |
    v
...
vigil complements (does not replace) existing tools. Its unique value is in detecting problems **specific to AI-generated code**.
...
Each finding includes `ruleId`, `level`, `location`, and optionally `fixes` with correction suggestions.
...
vigil is designed to be secure in enterprise environments:
...
- **Local cache**: Registry responses are cached in `~/.cache/vigil/registry/` (do not contain project data).
- **Auditable**: Every rule is a defined pattern in code. No black boxes.
...
No. vigil detects patterns **specific to AI-generated code** that other tools don't cover. It is complementary:
...
vigil is fast because it doesn't perform AI inference. A typical scan:
...
2. **Change the severity** so it doesn't block:
```yaml
rules:
  AUTH-005:
    severity: "low"
