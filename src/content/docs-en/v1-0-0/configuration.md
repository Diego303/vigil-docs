...
CLI flags always have the highest priority. This allows you to have a base configuration file for the team and make one-off adjustments per execution.
...
The search starts in the current directory and walks up the directory tree to the root. This allows you to have a configuration file at the monorepo root that applies to all subprojects.
...
deps:
  ...
  # Prevents making the same HTTP request twice in 24 hours
  cache_ttl_hours: 24
...
```yaml
auth:
  # Maximum lifetime hours for a JWT
...
secrets:
  ...
  # The default includes 32 patterns (see schema.py for the full list)
...
- The cache is shared across executions to avoid repeated requests.
...
There is no explicit option to disable it. You can use `cache_ttl_hours: 0` to force fresh requests on each execution, or `--offline` to make no requests at all.
