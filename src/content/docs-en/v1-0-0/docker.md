...
A more robust Dockerfile for production use or distribution:
...
For integration in projects using Docker Compose:
...
```bash
# First run: with network, to fill cache
docker run --rm \
  -v $(pwd):/scan \
  -v vigil-cache:/home/vigil/.cache/vigil \
  vigil deps
...
### With custom pre-built image

To avoid installing vigil on each run:
...
1. **Mount the code as read-only** (`-v $(pwd):/scan:ro`): vigil does not modify files. The read-only mount prevents accidental writes.
...
4. **Pin the vigil version**: In production Dockerfiles, use `pip install vigil-ai-cli==1.0.0` instead of `vigil-ai-cli` for reproducible builds.

5. **Separate scan from build**: In multi-stage builds, run vigil as an independent stage so it fails fast without wasting resources on subsequent stages.
