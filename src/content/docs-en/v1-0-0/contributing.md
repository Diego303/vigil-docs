...
tests/
  ...
  test_fase5_qa.py    # QA regression tests PHASE 5
  test_fase6_data_polish.py # Data loading and polish tests PHASE 6
  test_fase6_qa.py    # QA regression tests PHASE 6
...
data/                 # Static data
  ...
  placeholder_patterns.json # Placeholder patterns
scripts/              # Maintenance scripts
  fetch_popular_packages.py # Generates popular packages corpus
docs/                 # Documentation
```
...
Order: stdlib, third-party, local. Ruff takes care of sorting them.
...
2. **No side effects**: Don't modify files, don't write to stdout.
...
Add the rule to the rules documentation with:
...
- One logical assert per test (can be multiple `assert` if verifying the same thing).
...
| Data + Polish (PHASE 6) | 88 |
| QA regression (PHASE 6) | 100 |
| Logging | 3 |
| **Total** | **1706** |

Global coverage: **97%** (99% in reports module)
...
| **PHASE 6** | Popular packages corpus, final polish | Completed (QA done) |
| **V1 QA** | Exhaustive testing (164 manual tests) + 10 bug fixes | Completed |
