...
vigil includes 26 rules across 4 categories (24 implemented, 2 deferred). Each rule has a unique ID, default severity, and references to security standards when applicable.
...
**Why it's critical:** LLMs frequently generate package names that sound plausible but don't exist. An attacker can register that name with malicious code (slopsquatting). When someone runs `pip install` or `npm install`, they install the malicious package.
...
**Why it matters:** New packages haven't had time to be audited by the community. They could be malicious packages registered as part of a slopsquatting attack.

**Example:** A package created 3 days ago with a useful-sounding name (`fast-json-parser`) that nobody has reviewed.

**How to fix it:** Manually verify the package: review the source code, the maintainer, the purpose. If it's legitimate, add an exception in the config.
...
**What it detects:** The package name is very similar to a popular package (normalized Damerau-Levenshtein distance >= 0.85). Detects both substitution typos and adjacent character transposition.
...
**Why it matters:** Without access to the source code, it's impossible to audit what the package does. Legitimate packages almost always have a linked repository.
...
> **Status: IMPLEMENTED** — The `AuthAnalyzer` is active. All rules AUTH-001 through AUTH-007 are implemented. Supports Python (FastAPI/Flask) and JavaScript (Express).
...
**What it detects:** An endpoint handling sensitive data (users, payments, admin) without authentication middleware.
...
**What it detects:** JWT with lifetime greater than 24 hours (configurable with `auth.max_token_lifetime_hours`).
...
**What it detects:** Hardcoded JWT secret with a placeholder value or low entropy.
...
> **Status: IMPLEMENTED** — The `SecretsAnalyzer` is active. Rules SEC-001 through SEC-004 and SEC-006 are implemented. SEC-005 is deferred (requires .gitignore analysis). Supports Python and JavaScript/TypeScript.
...
**What it detects:** Hardcoded secret with low entropy (probably generated carelessly by the AI agent).
...
**What it detects:** Sensitive environment variable with a hardcoded default value in the code.
...
**What it detects:** Files containing credentials or keys that are not listed in `.gitignore`.
...
> **Status: IMPLEMENTED** — The `TestQualityAnalyzer` is active. All rules TEST-001 through TEST-006 are implemented. Supports pytest/unittest (Python) and jest/mocha (JavaScript/TypeScript).
...
**What it detects:** A test function that contains no assert, verify, expect, or similar.
...
**What it detects:** Assert that only verifies trivial conditions such as `is not None` or `assertTrue(True)`.
...
def test_user_exists():
    user = get_user(1)
    assert user is not None    # Trivial: doesn't verify the content
```
...
**What it detects:** Tests that make HTTP requests but don't verify the response status code.
...
**What it detects:** Mocks whose return value is exactly the value the test expects, creating a circular test that doesn't verify real logic.
