...
vigil uses a system of modular analyzers. Each analyzer focuses on a detection category and produces findings independently. This document describes the implemented analyzers.

For the general analyzer architecture (protocol, registration, flow), see [architecture](/vigil-docs/en/docs/v1-0-0/architecture/).
...
Verifies that each declared package exists in the public registry (PyPI or npm). If it doesn't exist, it's very likely a name hallucinated by the AI agent.
...
Checks the package's creation date. If it was created less than `deps.min_age_days` days ago (default: 30), it may be a malicious package registered as part of a slopsquatting attack.

**Requires network:** Yes. Skipped in `--offline` mode.

#### DEP-003 — Typosquatting candidate (HIGH)

Compares each dependency name against a corpus of popular packages using normalized Damerau-Levenshtein distance. If the similarity is >= `deps.similarity_threshold` (default: 0.85), it's a typosquatting candidate. Unlike classic Levenshtein, Damerau-Levenshtein counts the transposition of adjacent characters as a single operation, improving the detection of typos like `reqeusts` -> `requests`.
...
```
# requirements.txt
flask==99.0.0     # Version doesn't exist -> DEP-007 CRITICAL
```
...
|------|--------|----------|
| DEP-004 (unpopular) | Requires download statistics API, not available in basic PyPI/npm metadata | Future |
| DEP-006 (missing import) | Requires AST import parser, outside current scope (regex-based) | Future |
...
deps:
  ...
  # 0.85 = catches 1-character typos in names 8+ characters long
...
Detects insecure authentication and authorization patterns in Python (FastAPI/Flask) and JavaScript (Express) through regex pattern matching.
...
All rules are offline — they don't require network access. They only analyze source code.
...
Detects poorly managed secrets and credentials in code, with emphasis on patterns typical of AI-generated code: copied placeholders, low entropy secrets, and embedded `.env.example` values.
...
SecretsAnalyzer.analyze(files, config)
    |
    v
[1. Compile placeholder_patterns (32 regex)]
    |
    v
...
The analyzer comes with **32 known placeholder regex patterns**, configurable via `secrets.placeholder_patterns`:
...
- AWS/example values: `EXAMPLE`, `AKIA` (AWS example access key pattern)
...
The default threshold is 3.0 bits/char. Configurable with `secrets.min_entropy`.
...
3. Searches for those exact values in the source code.
...
secrets:
  ...
  # (list of 32 default patterns — see schema.py)
...
| Rule | Severity | Description |
|------|----------|-------------|
| TEST-001 | HIGH | Test without assertions (only verifies the code doesn't crash) |
...
All rules are offline — they don't require network access.
