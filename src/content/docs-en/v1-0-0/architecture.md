...
vigil-cli/
  src/vigil/
    ...
    analyzers/
      ...
      deps/                       # CAT-01: Dependency Analyzer
        ...
        similarity.py             # Damerau-Levenshtein + popular packages corpus
  ...
  tests/
    ...
    test_changed_only.py          # Tests for _get_changed_files() (11 tests)
    test_main_module.py           # Tests for python -m vigil and BaseAnalyzer protocol (8 tests)
    ...
    fixtures/                     # Test files
      ...
      secrets/                    # Secrets fixtures
        ...
        copies_env_example.py     #   Code that copies .env.example values
  ...
  data/
    ...
    placeholder_patterns.json     # 32 placeholder regex patterns
  scripts/
    fetch_popular_packages.py     # Script to regenerate popular packages corpus
```
...
3. Collects returned findings.
4. Catches exceptions per analyzer (a failed analyzer doesn't stop the others).
...
| `similarity.py` | Damerau-Levenshtein distance, PEP 503 normalization, popular packages corpus |
...
|------|-----------------|-------------|
| DEP-001 | Yes | Package doesn't exist in registry |
...
| DEP-007 | Yes | Pinned version doesn't exist |
...
1. Searches for the config file (manual with `--config`, or auto-detection going up the directory tree).
2. Parses the YAML.
...
| `show_suggestions` | `True` | Shows correction suggestions |
...
Compliant with SARIF 2.1.0. Includes `semanticVersion`, `defaultConfiguration` per rule, `helpUri`, `ruleIndex` in results, `snippet` in region, `invocations` with execution state and errors, and OWASP/CWE references in `properties`. Rule names are converted to PascalCase.
