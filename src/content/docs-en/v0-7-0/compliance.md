...
Organizations that adopt AI agents (Copilot, Cursor, Claude Code, ChatGPT) to generate code face new risks that traditional SAST tools don't cover:
...
- **Local cache**: Registry responses are cached in `~/.cache/vigil/registry/` (they don't contain project data).
...
vigil is fast because it doesn't do AI inference. A typical scan:
...
Yes. vigil supports multiple paths and file discovery does automatic pruning of directories like `node_modules/`, `.venv/`, etc.
