...
> **Never blindly trust AI agent output.** Treat generated code with the same scrutiny you would give a pull request from a junior developer who doesn't know your stack.
...
2. **Verify the exact name**: Agents frequently invent plausible names that don't exist.
...
# If there are critical findings, fix first
...
### Don't accept invented versions

Agents sometimes suggest versions that don't exist:
...
vigil detects this with rule DEP-007, but it's good practice to verify versions manually.
...
```python
# BAD — only verifies it doesn't crash
def test_login():
...
```python
# BAD — doesn't verify if the request failed
def test_get_users():
...
### Don't catch generic exceptions in tests
...
1. **Shared config**: Commit `.vigil.yaml` with the team config.
...
> "Generate the code using only packages that exist on PyPI. Verify that the names are correct. Don't invent packages."
...
> "Don't hardcode secrets or default values. All sensitive configurations must be read from environment variables without default values."

### For tests

> "Each test must have at least 2 meaningful assertions. Don't use assert is not None as the only assertion. Always verify the status code in API tests."
