...
4. **Pin the vigil version**: In production Dockerfiles, use `pip install vigil-ai-cli==0.7.0` instead of `vigil-ai-cli` for reproducible builds.
...
6. **Output to files within the volume**: When using `--output`, make sure the output file is in a mounted directory so you can access the report outside the container.
