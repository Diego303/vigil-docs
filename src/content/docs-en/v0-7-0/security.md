...
vigil detects all 4 problems from step 2. Since v0.6.0, all four analyzers are active: Dependency (DEP), Auth (AUTH), Secrets (SEC), and Test Quality (TEST).
...
| **Gitleaks/TruffleHog** | Real secret detection (API keys, tokens) | vigil detects placeholders and low-entropy secrets |
...
- **Business logic problems**: vigil looks for patterns, it doesn't understand logic.
...
- Are cached locally in `~/.cache/vigil/registry/` with configurable TTL (default 24h).
...
2. **Don't trust the agent for dependencies**: Always manually verify that suggested packages exist and are correct.
...
5. **Configure fail-on by environment**: Use `--fail-on critical` in local development and `--fail-on medium` in production CI.
...
3. **Audit the cache periodically**: Clean `~/.cache/vigil/registry/` if corrupt or stale data is suspected.

4. **Use the strict strategy for compliance**: The `strict` strategy aligns thresholds with SOC 2, ISO 27001, and EU CRA requirements.
