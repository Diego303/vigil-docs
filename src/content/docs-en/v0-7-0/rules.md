...
vigil V0 includes 26 rules in 4 categories. Each rule has a unique ID, default severity, and references to security standards when applicable.
...
**What it detects:** Hardcoded secret with low entropy (likely generated carelessly by the AI agent).
...
> **Status: IMPLEMENTED** — The `TestQualityAnalyzer` has been active since v0.6.0. All rules TEST-001 through TEST-006 are implemented. Supports pytest/unittest (Python) and jest/mocha (JavaScript/TypeScript).
...
**What it detects:** Assert that only verifies trivial conditions like `is not None` or `assertTrue(True)`.
...
**What it detects:** Mocks whose return value is exactly the value the test expects, creating a circular test that doesn't verify any real logic.
