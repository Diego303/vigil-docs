...
The findings appear in the **Security > Code scanning alerts** tab of the repository.
...
The findings appear in the merge request as failed tests in the **Tests** tab.
...
- Use `--quiet` to reduce output during the normal flow.
- Use `--offline` if you don't want the hook to depend on internet connectivity.
- Set `--fail-on critical` if the full scan is too strict for the local development flow.
...
An effective strategy is to start relaxed and gradually increase strictness:
...
- Runs all static checks that don't require network access.
- **Does run:** DEP-003 (typosquatting by name similarity -- no network required), and all static analyzers (AUTH-001..007, SEC-001..006, TEST-001..006).
