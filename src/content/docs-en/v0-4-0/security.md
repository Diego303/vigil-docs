...
This document describes the threat model of vigil, what it detects, how it aligns with industry standards, and what its limitations are.
...
vigil detects all 4 issues from step 2. Since v0.4.0, all four analyzers are active: Dependency (DEP), Auth (AUTH), Secrets (SEC), and Test Quality (TEST).
...
**Connection with LLMs:** AI agents frequently copy example values from documentation or generate predictable secrets when a user asks to "create a database configuration" or "generate a JWT token".
...
It is important to understand the limitations of vigil:
...
- **Business logic problems**: vigil looks for patterns, it does not understand logic.
- **Code in unsupported languages**: Currently only Python and JavaScript/TypeScript.

### vigil does not use AI

vigil is **deterministic**. It does not use LLMs, machine learning models, or statistical heuristics. Every rule is a defined pattern with explicit logic. This has advantages and drawbacks:
...
**Drawbacks:**
- Cannot detect novel problems not anticipated in the rules.
...
2. **Do not trust the agent for dependencies**: Always manually verify that suggested packages exist and are correct.
...
3. **Audit the cache periodically**: Clear `~/.cache/vigil/registry/` if you suspect corrupt or stale data.
