...
# Compliance and enterprise usage
...
| Adequate testing | TEST-001 to TEST-006 detect tests that verify nothing (since v0.4.0) |
...
vigil complements (does not replace) existing tools. Its unique value lies in detecting problems **specific to AI-generated code**.
...
The SARIF report maps each rule to CWEs where applicable:
...
- **Local cache**: Registry responses are cached at `~/.cache/vigil/registry/` (does not contain project data).
