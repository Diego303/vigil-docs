...
`--network none` disables container networking, ensuring no external requests are made. Combined with `--offline`, vigil runs only static checks.
...
4. **Pin the vigil version**: In production Dockerfiles, use `pip install vigil-ai-cli==0.6.0` instead of `vigil-ai-cli` for reproducible builds.
