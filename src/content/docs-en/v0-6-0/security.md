...
1. **Hallucinated dependencies (slopsquatting)**: The agent invents package names that do not exist. An attacker registers that name with malicious code. When someone runs `pip install` or `npm install`, the attacker's package gets installed.
...
- They do not compare values with `.env.example`.
...
5. On the next `pip install`, the malicious package gets installed
```

vigil detects all 4 issues from step 2. Since v0.6.0, all four analyzers are active: Dependency (DEP), Auth (AUTH), Secrets (SEC), and Test Quality (TEST).
...
**Connection with LLMs:** This is vigil's primary contribution. LLMs generate package names that sound plausible but do not exist ("python-jwt-utils", "fast-json-parser", "express-auth-middleware"). No other scanner verifies this.
...
- **Business logic issues**: vigil searches for patterns, it does not understand logic.
- **Code in unsupported languages**: Currently only Python and JavaScript/TypeScript.

### vigil does not use AI

vigil is **deterministic**. It does not use LLMs, machine learning models, or statistical heuristics. Each rule is a defined pattern with explicit logic. This has advantages and drawbacks:
...
5. **Configure fail-on per environment**: Use `--fail-on critical` in local development and `--fail-on medium` in production CI.
