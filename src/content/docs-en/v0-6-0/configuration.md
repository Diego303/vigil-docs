...
vigil is configured through a YAML file, CLI flags, or a combination of both. Configuration follows a three-layer model with progressive merge.
...
CLI flags always have the highest priority. This allows having a base configuration file for the team and making ad-hoc adjustments per execution.
...
The search starts in the current directory and walks up the directory tree to the root. This allows having a configuration file at the monorepo root that applies to all subprojects.
...
deps:
  ...
  # By default uses the corpus bundled in data/
...
rules:
  ...
  # Combine: enable with different severity
  TEST-002:
    enabled: true
    severity: "high"
```
...
```bash
# Example: dynamically generate config based on the environment
if [ "$CI" = "true" ]; then
...
vigil stores a cache of registry responses (PyPI/npm) at:
...
There is no explicit option to disable it. You can use `cache_ttl_hours: 0` to force fresh requests on each execution, or `--offline` to not make requests at all.
