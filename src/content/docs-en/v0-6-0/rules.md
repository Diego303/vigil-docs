...
**What it detects:** JWT secret hardcoded with a placeholder value or with low entropy.
...
**What it detects:** A hardcoded secret with low entropy (probably carelessly generated by the AI agent).
...
**What it detects:** An assert that only checks trivial conditions such as `is not None` or `assertTrue(True)`.
...
**What it detects:** Mocks whose return value is exactly the value the test expects, creating a circular test that does not verify real logic.
