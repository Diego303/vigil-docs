...
Specialized subcommand for analyzing dependencies. Runs only the `dependency` category rules.
...
Specialized subcommand for analyzing test quality. Runs only the `test-quality` category rules.
...
```bash
# Analyze the default test directory
vigil tests
