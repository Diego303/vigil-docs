...
For the general analyzer architecture (protocol, registration, flow), see [architecture](/vigil-docs/en/docs/v0-6-0/architecture/).
...
Files are automatically discovered with `find_and_parse_all()`, which traverses the directory tree while avoiding `.venv/`, `node_modules/`, `.git/`, etc.
...
**Corpus:** A built-in corpus of ~100 PyPI packages and ~70 npm packages is used as fallback. When the files `data/popular_pypi.json` and `data/popular_npm.json` are generated (PHASE 6), those will be used instead.
...
The `endpoint_detector` detects HTTP endpoints across three frameworks:
...
An endpoint is considered sensitive if its route contains tokens such as:
...
The analyzer comes with **30 regex patterns** for known placeholders, configurable via `secrets.placeholder_patterns`:
...
secrets:
  ...
  # Regex placeholder patterns for SEC-001
...
| Module | Responsibility |
|--------|----------------|
| `analyzer.py` | Orchestrates detection, iterates over test files and functions |
