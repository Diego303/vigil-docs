...
vigil-cli/
  ...
  tests/
    ...
    test_fase5_qa.py              # PHASE 5 QA regression (111 tests)
    test_core/
      test_finding.py
      test_engine.py
    ...
    fixtures/                     # Test files
      ...
      secrets/                    # Secrets fixtures
        ...
        .env.example              #   .env.example sample
      ...
      tests/                      # Test quality fixtures
        vulnerable_tests.py       #   Python tests with issues
...
| `similarity.py` | Levenshtein distance, PEP 503 normalization, popular packages corpus |
...
1. Finds the config file (manual with `--config`, or auto-detection by traversing up the directory tree).
2. Parses the YAML.
...
- `registry.by_severity(Severity.CRITICAL)` -- rules with a severity.
...
### Why dataclasses instead of Pydantic for Finding
...
- Dataclasses are lighter and faster for internally created data.
