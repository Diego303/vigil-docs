...
- Use `--offline` if you do not want the hook to depend on internet connectivity.
...
- Runs all static checks (patterns, imports, secrets, tests).
- Cannot verify if a package exists (DEP-001 is skipped).
- Cannot verify package age (DEP-002 is skipped).
