...
|--------|------------|------|---------|-------------|
| `--config` | `-c` | PATH | Auto-detect | Path to `.vigil.yaml` file |
...
| `--verbose` | `-v` | flag | false | Verbose output with debug logs |
...
# Save report to file AND show in terminal (human format only)
vigil scan src/ -o report.txt
```

### Output Behavior

- **`human` format**: If `--output` is specified, the report is written to the file AND shown in the terminal.
- **`json`, `junit`, `sarif` formats**: If `--output` is specified, the report is only written to the file. If not, it is shown on stdout.
...
```bash
# Analyze default tests directory
vigil tests
...
Lists all available rules with their descriptions, severities, and standards references.
