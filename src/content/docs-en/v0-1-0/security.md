---
title: "Security"
description: "Threat model, OWASP alignment, CWE references, and vigil's limitations."
order: 9
...
1. **Hallucinated dependencies (slopsquatting)**: The agent invents package names that don't exist. An attacker registers that name with malicious code. When someone runs `pip install` or `npm install`, they install the attacker's package.
...
- SAST tools don't verify that packages exist in registries.
- They don't detect slopsquatting or hallucinated dependencies.
- They don't evaluate the real quality of tests.
- They don't compare values with `.env.example`.
...
2. The agent generates code with:
   - A package that doesn't exist ("python-jwt-utils")
...
vigil detects all 4 problems from step 2.
...
- DEP-005: Dependencies without source repository
- DEP-006: Imports not declared in dependencies
- DEP-007: Non-existent versions

**Connection with LLMs:** This is vigil's main contribution. LLMs generate package names that sound plausible but don't exist ("python-jwt-utils", "fast-json-parser", "express-auth-middleware"). No other scanner verifies this.
...
vigil maps its rules to Common Weakness Enumerations (CWE) where applicable:
...
It's important to understand vigil's limitations:
...
| **Snyk/Dependabot** | Known vulnerabilities (CVE) | vigil detects packages that DON'T exist, Snyk detects vulnerabilities in packages that DO exist |
...
- **Business logic issues**: vigil looks for patterns, it doesn't understand logic.
- **Code in unsupported languages**: Currently only Python and JavaScript/TypeScript.

### vigil does not use AI

vigil is **deterministic**. It does not use LLMs, machine learning models, or statistical heuristics. Each rule is a pattern defined with explicit logic. This has advantages and drawbacks:
...
- Auditable: every rule is inspectable.
...
vigil makes HTTP requests to public registries (PyPI, npm) to verify package existence. These requests:

- Are read-only GET requests.
- Do not send project data (except package names).
- Can be completely disabled with `--offline`.
- Are cached locally in `~/.cache/vigil/registry/`.
...
- Contents: public package metadata (name, version, publication date, downloads).
...
vigil depends on trusted, well-established packages:
...
1. **Run vigil in CI**: Automate scanning on every PR so that no AI-generated code reaches production without verification.

2. **Don't trust the agent for dependencies**: Always manually verify that suggested packages exist and are the correct ones.
...
5. **Configure fail-on according to the environment**: Use `--fail-on critical` in local development and `--fail-on medium` in production CI.
...
3. **Audit the cache periodically**: Clean `~/.cache/vigil/registry/` if corrupted or stale data is suspected.
