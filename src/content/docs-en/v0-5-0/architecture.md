...
1. Finds the config file (manually with `--config`, or auto-detection by walking up the directory tree).
2. Parses the YAML.
...
- `registry.by_severity(Severity.CRITICAL)` -- rules at a severity level.
