...
For the general analyzer architecture (protocol, registration, flow), see [Architecture](/vigil-docs/en/docs/v0-5-0/architecture/).
...
Files are automatically discovered with `find_and_parse_all()`, which traverses the directory tree while skipping `.venv/`, `node_modules/`, `.git/`, etc.
...
Compares the name of each dependency against a corpus of popular packages using normalized Levenshtein distance. If the similarity is >= `deps.similarity_threshold` (default: 0.85), it is a typosquatting candidate.
...
**Corpus:** A built-in corpus of ~100 PyPI packages and ~70 npm packages is used as fallback. When the `data/popular_pypi.json` and `data/popular_npm.json` files are generated (PHASE 6), those will be used instead.
...
```yaml
auth:
  # Maximum JWT lifetime hours (AUTH-003)
...
| Rule | Severity | Description |
|------|----------|-------------|
| TEST-001 | HIGH | Test without assertions (only verifies that the code does not crash) |
