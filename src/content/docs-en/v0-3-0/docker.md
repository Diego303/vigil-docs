...
4. **Pin the vigil version**: In production Dockerfiles, use `pip install vigil-ai-cli==0.3.0` instead of `vigil-ai-cli` for reproducible builds.
...
6. **Output to files inside the volume**: When using `--output`, make sure the output file is in a mounted directory so the report can be accessed outside the container.
