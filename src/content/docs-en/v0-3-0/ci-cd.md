...
Now vigil runs automatically before each commit, scanning only changed files.
...
- **Does run:** DEP-003 (typosquatting by name similarity — no network required).
- **Does not run:** DEP-001 (nonexistent package), DEP-002 (new package), DEP-005 (no source repo), DEP-007 (nonexistent version).
- Future analyzers (auth, secrets, tests) are fully static and work in offline mode.
