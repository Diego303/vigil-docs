...
CLI flags always have the highest priority. This allows you to have a base configuration file for the team and make adjustments on a per-run basis.
...
deps:
  ...
  # Avoids making the same HTTP request twice within 24 hours
  cache_ttl_hours: 24
...
- The cache is shared across runs to avoid repeated requests.
...
There is no explicit option to disable it. You can use `cache_ttl_hours: 0` to force fresh requests on every run, or `--offline` to avoid making requests at all.
