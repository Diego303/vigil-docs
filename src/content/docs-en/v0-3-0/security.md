...
1. **Hallucinated dependencies (slopsquatting)**: The agent invents package names that do not exist. An attacker registers that name with malicious code. When someone runs `pip install` or `npm install`, the attacker's package is installed.
...
- They do not evaluate the real quality of tests.
...
2. The agent generates code with:
   ...
   - A hardcoded JWT secret ("supersecret123")
...
vigil detects all 4 issues from step 2. As of v0.3.0, the first three analyzers are active: Dependency (DEP), Auth (AUTH), and Secrets (SEC).
...
- Contains no sensitive project data.
...
3. **Review generated secrets**: Agents frequently generate placeholder secrets. Make sure environment variables are loaded from a real `.env` file and not from default values.
