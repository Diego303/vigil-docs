...
Detects hallucinated dependencies (slopsquatting), typosquatting, and suspicious packages. This is vigil's primary differentiator — no other scanner verifies that dependencies actually exist.
...
**Why it is critical:** LLMs frequently generate package names that sound plausible but don't exist. An attacker can register that name with malicious code (slopsquatting). When someone runs `pip install` or `npm install`, they install the malicious package.
...
**How to fix:** Search PyPI/npm for the correct package. In this case, the agent probably meant `PyJWT` or `python-jose`.
...
**How to fix:** Manually verify the package: review the source code, the maintainer, the purpose. If it is legitimate, add an exception in the config.
...
**How to fix:** Verify the correct package name. In the suggestion, vigil indicates which popular package it resembles.
...
> **Status: IMPLEMENTED** — The `AuthAnalyzer` is active since v0.3.0. All rules AUTH-001 through AUTH-007 are implemented. Supports Python (FastAPI/Flask) and JavaScript (Express).
...
**What it detects:** JWT with a lifetime greater than 24 hours (configurable with `auth.max_token_lifetime_hours`).
...
> **Status: IMPLEMENTED** — The `SecretsAnalyzer` is active since v0.3.0. Rules SEC-001 through SEC-004 and SEC-006 are implemented. SEC-005 is deferred (requires .gitignore analysis). Supports Python and JavaScript/TypeScript.
...
> **Status: PENDING** — Rules TEST-001 through TEST-006 are defined but the `TestQualityAnalyzer` is not yet implemented (PHASE 3).
...
**What it detects:** An assert that only verifies trivial conditions like `is not None` or `assertTrue(True)`.
