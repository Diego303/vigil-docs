...
description: "Recommendations for teams using AI agents to generate code and wanting to maintain security standards."
order: 12
...
> **Never blindly trust the output of an AI agent.** Treat generated code with the same scrutiny you would apply to a pull request from a junior developer who doesn't know your stack.
...
4. **Check downloads**: Packages with fewer than 100 weekly downloads deserve extra scrutiny.
...
vigil detects this with rule DEP-007, but it's good practice to manually verify versions.
...
```python
# BAD — the agent generates it like this
...
```bash
# Detect tests without assertions, trivial tests, and circular mocks
vigil tests
...
|------|---------|--------------|------|
| 1 | critical | DEP-001, DEP-007, SEC-001-003 | Only the most critical |
...
> "Don't hardcode secrets or default values. All sensitive configurations must be read from environment variables without a default value."
...
- [ ] Confirm that there are no values copied from `.env.example`.
