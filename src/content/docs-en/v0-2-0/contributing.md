...
Analyzers are the detection logic of vigil. Each analyzer implements the `BaseAnalyzer` protocol.
...
def _register_analyzers(engine: ScanEngine) -> None:
    ...
    from vigil.analyzers.secrets import SecretsAnalyzer  # new

    engine.register_analyzer(DependencyAnalyzer())
    engine.register_analyzer(SecretsAnalyzer())           # new
```
...
3. **Robust**: Catch I/O errors per file, not per full scan.
...
In the corresponding analyzer, add the logic that creates `Finding` with `rule_id="SEC-007"`.

### Step 3: Add tests

- Positive test: vulnerable code is detected.
- Negative test: safe code does not generate findings.
- Configuration test: the rule respects overrides.

### Step 4: Document

Add the rule to the [rules documentation](/vigil-docs/en/docs/v0-2-0/rules/) with:
...
| **PHASE 2** | Auth + Secrets analyzers (AUTH-001..007, SEC-001..006) | Pending |
