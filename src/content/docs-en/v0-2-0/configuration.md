---
title: "Configuration"
description: ".vigil.yaml file, strategies, overrides, and configuration merge."
order: 4
...
vigil is configured via a YAML file, CLI flags, or a combination of both. Configuration follows a three-layer model with progressive merging.
...
CLI flags always have the highest priority. This allows having a base configuration file for the team and making adjustments per execution.
...
The search starts in the current directory and goes up the directory tree to the root. This allows having a configuration file at the root of a monorepo that applies to all subprojects.
...
```yaml
deps:
  # Verify package existence on PyPI/npm via HTTP
  ...
  # TTL for registry response cache (in hours)
...
```yaml
auth:
  # Maximum JWT lifetime in hours
  # Tokens with a longer lifetime trigger AUTH-003
...
secrets:
  ...
  # Compare values in code with those in .env.example
...
```
project/
  .vigil.yaml            # Team's base config (committed)
...
```bash
# Example: dynamically generate config based on environment
if [ "$CI" = "true" ]; then
...
vigil stores registry response cache (PyPI/npm) at:
...
- The cache is shared between executions to avoid repeated requests.
...
There is no explicit option to disable it. You can use `cache_ttl_hours: 0` to force fresh requests on every execution, or `--offline` to not make any requests at all.
