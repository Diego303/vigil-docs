...
> **Status: IMPLEMENTED** — The `DependencyAnalyzer` is active. Rules DEP-001, DEP-002, DEP-003, DEP-005, and DEP-007 are implemented. DEP-004 and DEP-006 are deferred to a future release.
...
**Why it's critical:** LLMs frequently generate package names that sound plausible but do not exist. An attacker can register that name with malicious code (slopsquatting). When someone runs `pip install` or `npm install`, they install the malicious package.
...
**How to fix it:** Verify the correct package name. In the suggestion, vigil indicates which popular package it resembles.
...
> **Status: PENDING** — Rules AUTH-001 through AUTH-007 are defined but the `AuthAnalyzer` is not yet implemented (PHASE 2).
...
> **Status: PENDING** — Rules SEC-001 through SEC-006 are defined but the `SecretsAnalyzer` is not yet implemented (PHASE 2).
...
**What it detects:** A hardcoded secret with low entropy (probably generated by the AI agent without care).
